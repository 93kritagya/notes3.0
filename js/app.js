...
/* ── Hacker UI details panel ── */
function updateDetailsPanel(){
  const el=document.getElementById('details-content'); if(!el)return;
  const n=notes.find(x=>x.id===activeId);
  if(!n){el.innerHTML='<div class="details-empty">OPEN A NOTE<br><span>Select a file from the explorer.</span></div>';return;}
  const body=stripHtml(n.body||''); const words=body?body.trim().split(/\s+/).filter(Boolean).length:0;
  const mins=Math.max(1,Math.round(words/200)); const path=n.folderId?folderPath(n.folderId):'ROOT';
  const tags=n.tag?`<span class="detail-tag">#${esc(n.tag)}</span>`:'<span class="detail-tag">#untagged</span>';
  const related=notes.filter(x=>x.id!==n.id && ((n.tag&&x.tag===n.tag)||textForDetails(x).toLowerCase().includes((n.title||'').toLowerCase()))).slice(0,6);
  el.innerHTML=`
    <section class="details-section"><h3>NOTE DETAILS</h3><div class="details-grid">
      <span>Created</span><span>${new Date(n.created||n.updated||Date.now()).toLocaleString()}</span>
      <span>Modified</span><span>${new Date(n.updated||Date.now()).toLocaleString()}</span>
      <span>Word count</span><span>${words.toLocaleString()}</span>
      <span>Reading time</span><span>${mins} min</span>
      <span>Pinned</span><span>${n.pinned?'YES':'NO'}</span>
      <span>Favorite</span><span>${n.favorite?'YES':'NO'}</span>
    </div></section>
    <section class="details-section"><h3>TAGS</h3>${tags}</section>
    <section class="details-section"><h3>LOCATION</h3><div class="detail-link">▸ ${esc(path)}</div></section>
    <section class="details-section"><h3>BACKLINKS</h3><div id="details-backlinks">${related.length?related.map(x=>`<span class="detail-link" data-detail-open="${x.id}">▱ ${esc(x.title||'Untitled')}</span>`).join(''):'<span class="detail-link">No linked notes found.</span>'}</div></section>
    <section class="details-section"><h3>QUICK ACTIONS</h3>
      <button class="details-action" id="detail-ai-action">✦ Summarize / Generate questions</button>
      <button class="details-action" id="detail-history-action">◷ Open version history</button>
    </section>`;
  el.querySelectorAll('[data-detail-open]').forEach(b=>b.onclick=()=>openTab(Number(b.dataset.detailOpen)));
  document.getElementById('detail-ai-action')?.addEventListener('click',()=>document.querySelector('[data-ai="summary"]')?.click());
  document.getElementById('detail-history-action')?.addEventListener('click',()=>document.getElementById('history-btn')?.click());
}
function textForDetails(n){return stripHtml(n?.body||'')}
...
(function HakunaUpgrade(){
  ...
  renderEditor=function(){oldRenderEditor();refreshNoteBadges();updateDetailsPanel();};
  renderAll=function(){oldRenderAll();renderUpgradeList();updateDetailsPanel();};
  ...
  document.getElementById('explorer-new-folder')?.addEventListener('click',()=>newFolder());
  document.getElementById('explorer-new-note')?.addEventListener('click',()=>document.getElementById('new-btn')?.click());
  document.getElementById('details-history-btn')?.addEventListener('click',()=>document.getElementById('history-btn')?.click());
  document.getElementById('details-ai-btn')?.addEventListener('click',openAi);
